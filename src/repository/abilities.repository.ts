...
export class AbilitiesRepository {
  ...
  async findAll() {
    return (await this.database.ability.findMany()).map((ability) =>
      AbilityDbo.fromDb(ability).toModel(),
    );
  }
  ...
  async findByName(name: string): Promise<Ability | null> {
    const ability = await this.database.ability.findUnique({
      where: { name },
    });

    if (!ability) return null;

    return AbilityDbo.fromDb(ability).toModel();
  }
}
