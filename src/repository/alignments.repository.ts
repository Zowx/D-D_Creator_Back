...
export class AlignmentsRepository {
  ...
  async findAll() {
    return (await this.database.alignment.findMany()).map((alignment) =>
      AlignmentDbo.fromDb(alignment).toModel(),
    );
  }
  ...
  async findByName(name: string): Promise<Alignment | null> {
    const alignment = await this.database.alignment.findUnique({
      where: { name },
    });

    if (!alignment) return null;

    return AlignmentDbo.fromDb(alignment).toModel();
  }
}
